...
- Поток управления:
    - инкремент `AR_NEW` после каждой инструкции;
...
- `latch_reg_addr_new_signal` -- защелкнуть регистр `AR_NEW`
...
- `set_mux_addr_signal` -- установка мультиплексора на `ADDR`: 0 - из `DR`; 1 - из `AR_NEW`; 2 - из `reg_file`; 
